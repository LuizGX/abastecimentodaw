...
public class ErrosController implements ErrorController {
    private static final Logger logger = (Logger) LoggerFactory.getLogger(ErrosController.class);
    ...
    public String handleError(HttpServletRequest request) {
        ...
        if (status != null) {
            ...
            if(statusCode == HttpStatus.NOT_FOUND.value()) {
             logger.trace("A URL {} foi acessada mas não existe.", url);
                return "error/404";
            } else if(statusCode == HttpStatus.FORBIDDEN.value()) {
         logger.trace("Tentaram acessar a URL {} sem permissão.", url);
                return "error/403";
